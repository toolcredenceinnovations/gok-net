...
export function ProfileSettings() {
  ...
  async function saveProfile(event: FormEvent) {
    ...
    if (trimmedEmail !== (session.email ?? '')) {
      ...
      if (emailError) {
        toast.error(emailError.message || 'Could not update your email address.')
        setSavingProfile(false)
        return
      }
  ...
  async function changePassword(event: FormEvent) {
    ...
    if (!session.email) {
      toast.error('Your account has no email on file, so the password cannot be changed here.')
      return
    }
    ...
    <div className="product-page profile-page">
      ...
      <div className="profile-layout">
        <aside className="panel profile-summary">
          ...
          <ActivePill active className="profile-status" />
        </aside>
        ...
        <div className="profile-content">
          <form className="panel profile-section" onSubmit={saveProfile} noValidate>
            ...
            <div className="profile-form-grid">
              <label className="wide"><span>Full name</span><input value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" disabled={savingProfile} /></label>
              <label><span>Email address</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@company.com" autoComplete="email" disabled={savingProfile} /></label>
              <label><span>Phone number</span><input type="tel" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+91 98765 43210" autoComplete="tel" disabled={savingProfile} /></label>
            </div>
            <div className="profile-form-footer"><span>{profileSaved && <><CheckCircle2 size={15} /> Saved</>}</span><button className="primary-button" type="submit" disabled={!profileDirty || savingProfile}>{savingProfile ? 'Saving…' : 'Save profile'}</button></div>
          </form>
          ...
          <form className="panel profile-section" onSubmit={changePassword} noValidate>
            ...
            <div className="profile-form-grid password-grid">
              <label className="wide"><span>Current password</span><div className="password-input"><input type={showPasswords ? 'text' : 'password'} value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" disabled={changingPassword} /><button type="button" aria-label={showPasswords ? 'Hide passwords' : 'Show passwords'} onClick={() => setShowPasswords((value) => !value)}>{showPasswords ? <EyeOff size={16} /> : <Eye size={16} />}</button></div></label>
              <label><span>New password</span><input type={showPasswords ? 'text' : 'password'} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" disabled={changingPassword} /></label>
              <label><span>Confirm new password</span><input type={showPasswords ? 'text' : 'password'} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="new-password" disabled={changingPassword} /></label>
            </div>
            ...
            <div className="profile-form-footer"><span>{passwordChanged && <><CheckCircle2 size={15} /> Password updated</>}</span><button className="secondary-button" type="submit" disabled={changingPassword}>{changingPassword ? 'Updating…' : 'Change password'}</button></div>
          </form>
